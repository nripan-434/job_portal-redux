...
const JobCards = ({jobs}) => {
  ...
  return (
    <div className=' grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4  items-start'>
      {
        jobs?.map(x => {
          return <div className='flex flex-col gap-3  bg-[#d4d4d4] hover:bg-white duration-200 text-black rounded-sm m-3 overflow-hidden border' key={x._id}>
            <div className="flex items-center justify-between pr-3">
            <h1 className=' font-[impact] text-[30px]  pl-3 pt-3 hover:underline'>{x.companyname}</h1>
